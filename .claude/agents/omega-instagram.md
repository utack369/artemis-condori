...
- 禁止語：「効」「治」「消え」「改善」「変わ」「必ず」「絶対」「確実に」「限定」「今だけ」「残りわずか」「足のニオイ」「足が臭い」「悪臭」「臭い足」「足臭」「体臭」（語幹指定語は部分一致、その他は完全一致でチェック）
...
- 出力先（αへ明示渡し・鉄則3）：
  ...
  - 投稿用caption：~/Downloads/コンドリ/コンドリ動画自動生成/caption_[番号].txt
...
- A7：禁止語チェック（以下のルールで判定）
  - 語幹指定語（部分一致）：「効」「治」「消え」「改善」「変わ」→ナレーション・テロップ内に部分一致で含まれていないか
...
### C. 婉曲表現チェック
- C1：NGワード（prohibited_directに含まれるワードがナレーション・テロップに含まれていない）
- C2：変換の適切性（婉曲表現がconversion_tableの変換先と整合）
- C3：キャプション例外（キャプションは婉曲表現チェックの対象外）
...
- 段3でCoordinatorから以下を明示渡しで受領する。親文脈・α/βの生成文脈は継承しない。
  ...
  - 投稿用captionパス：~/Downloads/コンドリ/コンドリ動画自動生成/caption_[N].txt
