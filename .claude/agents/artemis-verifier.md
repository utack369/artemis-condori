...
### D群：心理導線
- **D1 入口の実装**(high)：psychology.entry がナレーション冒頭で実現（Type_3でentry=努力の肯定なのに緊張場面から始まる等はfail）。psychology.entry が指示書で未定義の場合は必ずskipとし、判定理由に「entry未定義」と明記する。
- **D2 出口の実装**(high)：psychology.exit がナレーション末尾・テロップ全体に反映（Type_2でexit=安心なのに方向転換で終わる等はfail）。psychology.exit が指示書で未定義の場合は必ずskipとし、判定理由に「exit未定義」と明記する。
...
- **E2 CTA非存在時の混入**(high／cta_required=falseのみ)：fail＝テロップおよびcaptionに、(1)特定行動（フォロー・保存・いいね・コメント・シェア・リンク遷移・DM）への命令形による指示、または(2)フォロー・いいね・コメント・シェア・リンク遷移・DMへの依頼形・勧誘形による指示がある。pass＝陳述形の存在言及（例「プロフィールにまとめています」）・現実世界や内省への提案（例「振り返ってみてください」「参考にしてみてください」）・状況描写・自問。保存のみ例外として、「よかったら」等の緩和を伴う依頼形の保存促し（例「よかったら保存しておいてください」）は許容する（ep34・ep54前例・2026-08-06うた裁定）。境界事例はfailにせずneeds_reviewとし、Coordinator経由でうたへエスカレーションする。
...
| E2 CTA非存在 | **全スライドおよびcaption**を対象に、reel節のE2基準を適用（命令形の特定行動指示はfail／フォロー・いいね・コメント・シェア・リンク遷移・DMは依頼形・勧誘形もfail／陳述形の存在言及と現実世界・内省への提案はpass／緩和依頼形の保存促しのみ許容＝ep34・ep54前例）。境界はneeds_reviewでエスカレーション |
