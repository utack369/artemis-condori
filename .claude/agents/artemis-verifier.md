...
- **E2 CTA非存在時の混入**(high／cta_required=falseのみ)：fail＝テロップおよびcaptionに特定行動（フォロー・保存・いいね・コメント・シェア・リンク遷移・DM）への命令形・依頼形・勧誘形による直接指示がある。pass＝直接指示を含まない存在言及・状況描写・自問（婉曲なプロフィール言及・保存促しは許容）。境界事例はfailにせずneeds_reviewとし、Coordinator経由でうたへエスカレーションする。
...
| E2 CTA非存在 | **全スライドおよびcaption**に特定行動（フォロー・保存・いいね・コメント・シェア・リンク遷移・DM）への直接指示（命令形・依頼形・勧誘形）がない。婉曲な言及は許容。境界はneeds_reviewでエスカレーション |
