...
```
- mp4/png は cp（Downloads の投稿用原本を温存）、指示書JSON は cp（input/ の原本を温存）、caption_[N].txt は cp（output/ 側の正本を Downloads へ複製）。
