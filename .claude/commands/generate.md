...
### 段2-検証：selected_text機械検証
`python3 scripts/check_selected_text.py input/instagram/ep[N].json` を実行する。exit 1 の場合、Coordinator 経由で omega① へ差し戻し selected_text を再生成させる（最大2回）。2回を超えて不合格の場合はうたへエスカレーションして停止する。

## 段3：原稿・サムネ指示の生成と品質ゲート
指示書JSON を入力に alpha が caption.md / script.md（format=reel）またはcarousel_slides.md（format=carousel）/ thumbnail-instruction.md を生成。formatは ep[N].json の meta.format で判定する。Hook形式ゲートを通過させ、beta が品質チェックを行う。NG の場合は Coordinator 経由で alpha へ自動差し戻し（最大2回）。差し戻しが2回超過、または confidence < 0.7 の場合はうたへエスカレーションして停止。成果物は output/instagram/ep[N]/ に出力し、投稿用 caption_[N].txt も output/instagram/ep[N]/ に生成する（正本はここ。artemis-media側への複製は段5のcpで行う）。差し戻しで selected_text を変更した場合は、ep[N].json と thumbnail-instruction.md の両方を同期修正すること。
...
- 検証対象：output/instagram/ep[N]/ の script.md（format=reel）またはcarousel_slides.md（format=carousel）/ caption.md（beta 確定物・検証正本）。caption_[N].txt は投稿用の別形式のため検証対象外。
...
### 段3-検証（再）：selected_text最終確認
段4に進む前に `python3 scripts/check_selected_text.py input/instagram/ep[N].json` を再実行する（差し戻しで json が変更された場合の最終確認）。exit 1 の場合は段4に進まず停止し、うたへ報告する。

## 段4：動画生成（format=reel）／スライド画像生成（format=carousel）

**format=carouselの場合**、以下を実行する（動画生成は行わない）：
```
cd ~/artemis-condori
N=$ARGUMENTS
python3 scripts/generate_carousel.py "$N" "output/instagram/ep${N}/carousel_slides.md" "output/instagram/ep${N}/slides"
```
- 出力：output/instagram/ep[N]/slides/ に slide_[N]_01.png 〜（7〜8枚、1080x1350）
- slide_[N]_01.png（表紙）がカルーセルの1枚目＝サムネイル相当を兼ねる
- 生成後、スライド枚数が carousel_slides.md のブロック数と一致することを確認する

**format=reelの場合**、以下を実行する：
```
cd ~/artemis-condori
N=$ARGUMENTS
...
**format=carouselの場合**、以下を実行する（artemis-mediaへの複製は不要。投稿はslides/を直接参照する）：
```
cd ~/artemis-condori
N=$ARGUMENTS
...
## 完了
output/instagram/ep[N]/ の集約物一覧（caption.md / script.md または carousel_slides.md / thumbnail-instruction.md / caption_[N].txt / ep[N].json / 動画または画像群 / png）をうたに提示して停止する。push は行わない。
